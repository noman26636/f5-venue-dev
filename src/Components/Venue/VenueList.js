...
const VenueList = () => {
		...
		if (venues && venues?.length > 0) {
			venues.forEach((venue) => {
                ...
                <a href="/venue/${venue.id}">
                    <img src=${
											venue.images?.length > 0 ? venue.images[0].image_path_thumbnail : null
										} class="popup-img">
                </a>
                    <div>
                      <a href="/venue/${venue.id}" >  
	...
	useEffect(() => {
		...
		if (searchData) {
			...
			valuesObj.seatingOption = searchData.seatingOption ? Number(searchData.seatingOption) : 0
			valuesObj.moreFilters = searchData.moreFilters?.length > 0 ? [...searchData.moreFilters] : []
			valuesObj.name = searchData.name ? searchData.name : ''
			valuesObj.sortField = searchData.sortField ? Number(searchData.sortField) : -1
			valuesObj.sortType = searchData.sortType ? Number(searchData.sortType) : 0
	...
	const searchVenues = (searchParams = values, pageNumber) => {
		...
		if (searchParams) {
			...
			if (searchParams.capacity > 0)
				searchObj.people = [enum_seatingOptions[Number(searchParams.seatingOption)], Number(searchParams.capacity)]
			if (searchParams.eventType?.length > 0) searchObj.type = searchParams.eventType?.map((item) => Number(item.id))
			if (searchParams.moreFilters?.length > 0) searchObj.service = searchParams.moreFilters?.map((item) => item.id)
			if (searchParams.name && searchParams.name !== '') searchObj.name = searchParams.name
					...
					<Row className="venue-details-block">
						{venuesList?.length < 1 ? (
							...
							venuesList?.map((item, i) => (
								...
								>
									<div className="image-block">{<img alt="" src={item.images[0]?.image_path_thumbnail} />}</div>
										...
										<div className="rating-block">
											...
											{item.ratings?.length > 0 && (
												<span className="reviews">
													{item.ratings.length}{' '}
													{item.ratings?.length !== 1 ? translations.Reviews : translations.Review}
												</span>
											)}
										</div>
										<div className="d-flex">
